    ...
    async with aiohttp.ClientSession() as session:
        for page in pages:
            async with session.get(
                    f"https://catalog.wb.ru/catalog/men_clothes2/"
                    f"catalog?appType=1&couponsGeo=12,3,18,15,21&curr=rub&"
                    f"dest=-1029256,-102269,-2162196,-1257786&emp=0&kind=1&"
                    f"lang=ru&locale=ru&page={page}&pricemarginCoeff=1.0&"
                    f"reg=1&regions=80,64,83,4,38,33,70,82,69,68,86,75,30,40,"
                    f"48,1,22,66,31,71&sort=popular&spp=28&sppFixGeo=4&"
                    f"subject=177;284;4853",
                    headers=headers,
            ) as response:
